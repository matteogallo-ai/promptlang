...
export function formatTerminalReport(
  ...
  if (errors.length > 0) {
    ...
    for (const issue of errors) {
      ...
      lines.push(`    ${RED}[${issue.rule}]${RESET} ${issue.message}`);
      if (issue.suggestion) {
  ...
  if (warnings.length > 0) {
    ...
    for (const issue of warnings) {
      lines.push("");
      lines.push(`  ${WHITE}${issue.file}:${issue.line}${RESET}`);
      lines.push(`    ${YELLOW}[${issue.rule}]${RESET} ${issue.message}`);
      if (issue.suggestion) {
        lines.push(`    ${DIM}${issue.suggestion}${RESET}`);
      }
    }
  ...
  if (infos.length > 0) {
    ...
    for (const issue of infos) {
      lines.push("");
      lines.push(`  ${WHITE}${issue.file}:${issue.line}${RESET}`);
      lines.push(`    ${BLUE}[${issue.rule}]${RESET} ${issue.message}`);
      if (issue.suggestion) {
        lines.push(`    ${DIM}${issue.suggestion}${RESET}`);
      }
    }
  ...
  if (issues.length === 0) {
    lines.push("");
    lines.push(`${BOLD}✓  No issues found.${RESET}`);
  }
